"""
クライアントと Azure OpenAI サービス間のリアルタイム通信を処理するミドルウェア RTMiddleTier を定義します。
ツールの呼び出しやメッセージのフィルタリング、セッションの設定など、通信の中継と加工の役割を担います。
"""
...
# ツールの結果がサーバ側で処理されるか、クライアント側で送信されるかを示す列挙型
...
ツールの実行結果を保持するクラス
text: ツールの結果のテキストを格納
destination: ツールの結果がサーバ側で処理されるか、クライアント側で送信されるかを格納
"""
...
"""
ツールの呼び出し情報を保持するクラス
tool_call_id: ツールの呼び出しの一意な識別子
previous_id: 前のメッセージの識別子
"""
...
class RTMiddleTier:
    endpoint: str  # Azure OpenAI サービスのエンドポイント URL
    deployment: str  # 使用するモデルのデプロイ名
    key: Optional[str] = None  # Azure OpenAI API キー
    ...
    # Azure OpenAI サービスから受信したメッセージをクライアントに送信する前に処理
    # https://platform.openai.com/docs/api-reference/realtime-server-events
    async def _process_message_to_client(
        self,
        msg: str,  # 受信したメッセージ
        client_ws: web.WebSocketResponse,  # クライアントとの WebSocket 接続
        server_ws: web.WebSocketResponse,  # Azure OpenAI サービスとの WebSocket 接続
    ) -> Optional[str]:
        message = json.loads(msg.data)  # 受信したメッセージを JSON 形式に変換
        updated_message = msg.data  # 更新後のメッセージを初期化
        if message is not None:
            match message["type"]:
                ...
                # https://platform.openai.com/docs/api-reference/realtime-server-events/response/output_item/added
                case "response.output_item.added":
                    # メッセージが function_call タイプの場合、クライアントに転送しない
                ...
                case "conversation.item.created":
                    if "item" in message and message["item"]["type"] == "function_call":
                        item = message["item"]
                        # function_call タイプの場合、ツールの呼び出しを検出し、_tools_pending に追加
    ...
    # クライアントから受信したメッセージを Azure OpenAI サービスに送信する前に処理
